...
const options = require('./dbOptions.json');
...
module.exports = class Datastorage {
    ...
    insert(resourceObject) {
        return new Promise ( async (resolve, reject) => {
            try {
                this.db.doQuery(insertSql, toInsertArray(resourceObject));
            ...
            try {
                if (key && resourceObject) {
                    ...
                    if (resultGet.queryResult.length > 0) {
                        ...
                        this.insert(resourceObject).then(status => resolve(status)).catch(err => reject(err));
                    }
                }
                else {
                    resolve(MESSAGES.NOT_UPDATED());
